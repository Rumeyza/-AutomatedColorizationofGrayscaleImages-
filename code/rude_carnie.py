...
def _convert_to_example(filename, image_buffer, label_age, label_gender, height, width):
    ...
    example = tf.train.Example(features=tf.train.Features(feature={
        'image/class/label_age': _int64_feature(label_age),
        'image/class/label_gender': _int64_feature(label_gender),
...
def _process_image_files_batch(coder, thread_index, ranges, name, filenames,
                               labels_age, labels_gender, num_shards):
    ...
    for s in xrange(num_shards_per_batch):
        ...
        for i in files_in_shard:
            ...
            label_age = int(labels_age[i])
            label_gender = int(labels_gender[i])
            ...
            example = _convert_to_example(filename, image_buffer, label_age, label_gender,
                                          height, width)
...
def _process_image_files(name, filenames, labels_age,labels_gender, num_shards):
    ...
    num_shards: integer number of shards for this data set.
    """
    assert len(filenames) == len(labels_age)
    assert len(filenames) == len(labels_gender)
    ...
    for thread_index in xrange(len(ranges)):
        args = (coder, thread_index, ranges, name, filenames, labels_age,labels_gender, num_shards)
...
def _find_image_files(list_file, data_dir):
   ...
   # print(files_labels)
    ...
    for path, label_age, label_gender in files_labels:
    ...
    unique_labels_age = set(labels_age)
    unique_labels_gender = set(labels_gender)
    ...
    print('Found %d JPEG files across %d/%d labels inside %s.' %
          (len(filenames), len(unique_labels_age), len(unique_labels_gender), data_dir))
...
def _process_dataset(name, filename, directory, num_shards):
    ...
    """
    filenames, labels_age, labels_gender = _find_image_files(filename, directory)
    _process_image_files(name, filenames, labels_age, labels_gender, num_shards)
    unique_labels_age = set(labels_age)
    unique_labels_gender = set(labels_gender)
    assert len(labels_age) == len(labels_gender)
    return len(labels_age), unique_labels_age, unique_labels_gender
...
def main(unused_argv):
    ...
    # Run it!
    valid, valid_outcomes_age, valid_outcomes_gender = _process_dataset('validation', '%s/%s' % (FLAGS.fold_dir, FLAGS.valid_list), FLAGS.data_dir,
                     FLAGS.valid_shards)
    train, train_outcomes_age, train_outcomes_gender = _process_dataset('train', '%s/%s' % (FLAGS.fold_dir, FLAGS.train_list), FLAGS.data_dir,
                     FLAGS.train_shards)
    
    if len(valid_outcomes_age) != len(valid_outcomes_age | train_outcomes_age):
        print('Warning: unattested labels in training data [%s]' % (', '.join((valid_outcomes_age | train_outcomes_age) - valid_outcomes_age)))
    ...
    md = { 'num_valid_shards': FLAGS.valid_shards, 
           ...
           'nlabels_age': len(train_outcomes_age),
           'nlabels_gender': len(train_outcomes_gender)}
